    ...
    def __init__(self, argument):
        ...
        if not status.hasDateOrTime:
            raise commands.BadArgument(
                'invalid time provided, try e.g. "20minutes" or "3d"'
            )
...
class UserFriendlyTime:
    ...
    def __init__(self, default=None, date_override=None):
    ...
    async def convert(self, argument):
        ...
        if self.date_override is None:
            now = datetime.datetime.now(datetime.timezone.utc)
        else:
            now = self.date_override
        ...
        if elements is None or len(elements) == 0:
            raise commands.BadArgument(
                f'Invalid time provided, try e.g. "tomorrow" or "3 days". You provided: {argument}'
            )
